...
const loadUserProfile = async () => {
  try {
    ...
    console.log("DEBUG - Checking all paths:");
    console.log("responseData.registration_info:", responseData.registration_info);
    console.log("responseData.registration_data:", responseData.registration_data);
    console.log("responseData.selected_market (root):", responseData.selected_market);
    console.log("responseData.selected_strategies (root):", responseData.selected_strategies);
    console.log("responseData.market_preferences:", responseData.market_preferences);
  ...
  const handleUpdateProfile = async () => {
    try {
      // Note: Since backend only has GET endpoint for users,
      // we're updating localStorage only for now
      ...
      localStorage.setItem("userName", `${userProfile.firstName} ${userProfile.lastName}`.trim());
      
      // Notify Navbar to update immediately
      window.dispatchEvent(new Event("storage"));
      
      alert("Profile updated successfully in local storage!");
  ...
  try {
    ...
    console.log("Browser offset:", date.getTimezoneOffset(), "minutes");
    ...
    console.log("IST time (forced):", istTime);
              ...
              {activeTab === "account" && (
                <Card className="bg-slate-900/40 border-slate-800 text-white">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {/* NEW: Original Registration Preferences */}
                    <div className="space-y-1">
                      <label className="text-xs text-slate-500 uppercase font-bold flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* Original Strategies - Multi-line layout */}
<div className="space-y-1">
          ...
          Strategies may have been selected later
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* US Market */}
                        <div className={`p-4 rounded-lg border ${userProfile.marketPreferences.US.is_active ? 'border-green-500/50 bg-green-500/5' : 'border-slate-700 bg-slate-800/30'}`}>
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <Building2 className="w-4 h-4 text-cyan-400" />
